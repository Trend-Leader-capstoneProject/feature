...
import { useSaveInterests } from "../../src/features/interest/hooks/useSaveInterests";
import { InterestSelectScreen } from "../../src/features/interest/screens/InterestSelectScreen";
import type { CategoryListData } from "../../src/features/interest/types/category";
import type { InterestSaveErrorResponse } from "../../src/features/interest/types/interest";
...
const mockedUseSaveInterests = jest.mocked(useSaveInterests);
...
type MutationCallbacks = {
  onSuccess?: () => void;
  onError?: (error: AxiosError<InterestSaveErrorResponse>) => void;
};
...
function getMutationCallbacks(): MutationCallbacks {
  ...
  if (!call) {
    throw new Error("관심사 저장 Mutation이 호출되지 않았습니다.");
  }

  return call[1] as MutationCallbacks;
}

async function submitInterestSelection(): Promise<MutationCallbacks> {
...
describe("InterestSelectScreen auth session integration", () => {
  ...
  beforeEach(() => {
    ...
    mockedUseSaveInterests.mockReturnValue({
      ...
      isPending: false,
      isSuccess: false,
    } as unknown as ReturnType<typeof useSaveInterests>);
  });
  ...
  test("관심사 저장 201 성공 시 completeInterestSelection을 호출한다", async () => {
    const callbacks = await submitInterestSelection();

    callbacks.onSuccess?.();

    expect(completeInterestSelectionMock).toHaveBeenCalledTimes(1);

    expect(revalidateSessionMock).not.toHaveBeenCalled();
  });

  test("관심사 저장 409 발생 시 Session을 재검증한다", async () => {
    const callbacks = await submitInterestSelection();

    callbacks.onError?.(createInterestSaveError(409));
  ...
  test("관심사 저장 401은 화면 자체의 인증 Alert를 표시하지 않는다", async () => {
    const callbacks = await submitInterestSelection();

    callbacks.onError?.(createInterestSaveError(401));
