...
const INTEREST_SELECTION_SESSION:
  SessionResponse = {
    user: {
      user_id: 1,
      login_id: "user_a",
...
describe(
  "AuthProvider session transition",
  () => {
    ...
    test(
      "관심사 선택 완료 시 현재 Session을 MAIN으로 변경한다",
      async () => {
        const {
          result,
        } =
    ...
    test(
      "동일 Token의 Session 재검증 결과는 현재 Session에 반영한다",
      async () => {
        const {
          result,
        } =
    ...
    test(
      "TOKEN_A 재검증 중 TOKEN_B가 생성되면 TOKEN_A의 늦은 응답을 무시한다",
      async () => {
        const {
          result,
        } =
        ...
        const deferredSession =
          createDeferred<SessionResponse>();
    ...
    test(
      "Session 종료 중 도착한 재검증 응답은 종료 중인 Session을 갱신하지 않는다",
      async () => {
        const {
          result,
        } =
