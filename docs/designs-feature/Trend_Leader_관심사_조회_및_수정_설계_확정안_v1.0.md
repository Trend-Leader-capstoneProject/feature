# Trend Leader 관심사 조회 및 수정 설계 확정안 v1.0
...
`InterestService`는 한 사용자의 관심사 집합에 대한 Use Case를 소유한다.
...
PUT 변경 작업 중 다음 어느 시점에서 SQLAlchemy DB 오류가 발생해도:
...
알 수 없는 DB 오류를 임의로 Domain `409`로 변환하지 않는다.
...
SQLAlchemy의 실제 표현은 구현 시작 시 현재 설치된 SQLAlchemy 버전과 공식 API를 확인한 뒤 사용한다.

### 설계 목표

같은 사용자의 관심사 수정 Transaction은 서로 직렬화되어야 한다.
...
find_by_user_id_for_update()
→ 현재 사용자 Interest 조회
→ 실제 Lock 사용 여부는 Transaction 테스트로 검증
...
동시성 테스트는 Testcontainers/MariaDB Integration 환경에서 가능한 범위를 구현 단계에서 검토한다.
...
8. commit / rollback
9. Service Unit Test
10. Transaction Integration Test
11. Race Condition 검증
```
...
| 수정 동시성 | 사용자 Interest Row `FOR UPDATE` 계열 Lock |
...
> 관심사 수정은 인증 Session을 다시 만드는 기능이 아니라, 이미 Onboarding을 완료한 사용자의 비어 있지 않은 관심사 집합을 PUT으로 원자적으로 교체하는 기능이며, API는 전체 상태를 전달하되 Backend는 기존 Row를 보존하는 diff 방식으로 수정하고 Frontend는 Query Server State와 편집 Draft State를 분리한다.

이 원칙을 유지하면 최초 관심사 저장, 인증 Session, 향후 관심사 기반 Trend 추천 사이의 책임을 분리한 채 기능을 확장할 수 있다.
