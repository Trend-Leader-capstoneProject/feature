...
1. Category는 주제 영역만 표현한다.

2. 사용자의 관심사는 대분류이다.

3. Trend에는 여러 구체적인 세부분류를 연결할 수 있다.

4. Trend에는 부모 대분류를 중복 매핑하지 않는다.

5. 추천에서는 사용자 관심 대분류의
   하위 세부분류까지 탐색한다.

6. 서로 다른 부모 아래 동일한 category_name을 허용한다.

7. 초기에는 기타 Category를 fallback으로 사용한다.

8. Hashtag는 Category와 분리한다.

9. MVP Hashtag는 AI가 생성하고 검색/탐색에 활용한다.

10. 신제품/업데이트/이벤트/밈 등은
    Category가 아니라 별도 Attribute 후보이다.
...
11. 여러 Category 중 대표 Category를 지정할지는 보류한다.

12. 대표 Category의 실제 Use Case가 생기면
    선정 기준을 별도로 설계한다.

13. 현재 is_primary DB 필드의 제거 여부도
    지금 결정하지 않는다.
