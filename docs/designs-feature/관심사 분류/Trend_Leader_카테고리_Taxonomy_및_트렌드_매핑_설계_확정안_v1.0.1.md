# Trend Leader 카테고리 Taxonomy 및 트렌드 매핑 설계 확정안 v1.0.1

- 프로젝트: Trend Leader
- 기준 Repository: `Trend-Leader-capstoneProject/feature`
- 기준 브랜치: `dev`
- 기준일: 2026-09-13
- 기반 문서: `Trend_Leader_카테고리_Taxonomy_및_트렌드_매핑_설계_확정안_v1.0.md`
- 변경 사항: Section 5.9의 기존 데이터 불일치·모호한 식별·Transaction rollback 정책 보강
- 상태: 설계 확정
...
#### 5.9.1 기존 Category와 Seed 정의가 불일치하는 경우

Seed 식별 기준으로 동일한 Category가 이미 존재하더라도 DB에 저장된 값이
Category Master Seed v1.0 정의와 다를 수 있다.
...
이 경우 Category Master Seed는 기존 Row를 Seed 정의에 맞게 자동 UPDATE하지 않는다.

최종 정책은 다음과 같다.

```text
식별 기준으로 Category가 존재하지 않음
→ 신규 생성

식별 기준으로 동일 Category가 존재하고
Seed 정의와 검증 대상 값이 모두 일치함
→ 기존 Row 유지
→ 중복 생성하지 않음

식별 기준으로 동일 Category가 존재하지만
Seed 정의와 검증 대상 값이 하나라도 불일치함
→ 자동 UPDATE 금지
→ Seed 실행 실패
→ 전체 Transaction rollback
→ 불일치 내용을 명시적으로 확인

Seed 정의에 없는 기존 Category
→ 자동 DELETE 금지
```

불일치 검증 대상은 최소 다음을 포함한다.
...
category_name
category_code = NULL
parent_id = 식별된 부모 대분류
sort_order
is_active = true
```

#### 5.9.2 Seed 식별이 모호한 기존 데이터

Seeder가 하나의 Seed 항목에 대해 기존 DB Row를 유일하게 식별할 수 없는 경우
임의의 Row를 선택하거나 새로운 Row를 추가하지 않는다.
...
이와 같이 동일 부모 아래 동일한 세부분류가 중복되어 있거나,
그 밖의 비정상적인 데이터로 Seed 대상 Row를 유일하게 결정할 수 없는 경우
다음 정책을 적용한다.
...
Seeder는 기존 데이터 이상을 숨기는 복구 도구가 아니라,
Category Master Seed v1.0을 안전하게 생성하고 검증하는 도구로 취급한다.

#### 5.9.3 명시적 변경과 Transaction 정책

Category rename, deactivate, merge 또는 운영 정책에 따른 `sort_order` 변경은
초기 Seed 재실행으로 암묵적으로 처리하지 않고 별도의 명시적인 데이터 변경 절차로 수행한다.

Seed 실행은 하나의 명시적인 Transaction으로 처리한다.
기존 데이터 불일치나 모호한 중복을 포함하여 실행 도중 오류가 발생하면,
해당 실행에서 앞서 생성한 Category까지 모두 rollback하여 부분 Seed 상태를 남기지 않는다.
